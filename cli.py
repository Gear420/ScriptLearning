...
for file_path in l:
    ...
    #    ids_start = model.embeddings.word_embeddings.weight.shape[0]
    #    special_tokens = ['<t1>', '<t2>', '<t3>', '<t4>']
    #    num_added_tokens = tokenizer.add_special_tokens({'additional_special_tokens': special_tokens})
    #    model.resize_token_embeddings(ids_start + num_added_tokens)
...
def construct_generation_args():
    ...
    parser.add_argument("--model_name", type=str, default='gpt2', choices=SUPPORT_MODELS)
    ...
    # directories
    parser.add_argument("--data_dir", type=str, default=join(abspath(dirname(__file__)), 'LAMA'))
...
class Trainer(object):
    def __init__(self, args):
        self.args = args
        self.device = 'cuda:2' if self.args.model_name != 't5-11b' else 'cuda:{}'.format(self.args.t5_shard * 4)
        ...
        self.train_loader = DataLoader(self.train_set, batch_size=1, shuffle=True, drop_last=True)
        self.dev_loader = DataLoader(self.dev_set, batch_size=1)
    ...
    def train(self):
        ...
        for epoch_idx in range(100):
            ...
            tot_loss = 0
            # for i in range(len(data_loader[:5])):
            #     self.model.train()
            #     print("train_loader:")
            #     print(data_loader[i])
            #     loss, batch_hit1 = self.model(data_loader[i], 'before')
            #     # print(loss)
            #     # print(batch_hit1)
            #     hit1 += batch_hit1
            #     tot_loss += loss.item()
